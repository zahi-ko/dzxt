...
def test_upload_then_list(client: TestClient) -> None:
    response = client.post("/api/audio/upload", files={"file": ("tone.wav", wav_bytes(), "audio/wav")})
    assert response.status_code == 200
    meta = response.json()
    assert meta["duration"] == pytest.approx(0.5, rel=0.05)

    listed = client.get("/api/audio").json()["items"]
    assert len(listed) == 1
    assert listed[0]["audio_id"] == meta["audio_id"]


def test_apply_effect_creates_new_handle(client: TestClient) -> None:
    audio_id = client.post(
        "/api/audio/upload", files={"file": ("t.wav", wav_bytes(), "audio/wav")}
    ).json()["audio_id"]
...
def test_peaks_returns_envelope(client: TestClient) -> None:
    audio_id = client.post(
        "/api/audio/upload", files={"file": ("t.wav", wav_bytes(1.0), "audio/wav")}
    ).json()["audio_id"]
...
def test_upload_rejects_garbage(client: TestClient) -> None:
    response = client.post("/api/audio/upload", files={"file": ("bad.wav", b"not-a-wav", "audio/wav")})
