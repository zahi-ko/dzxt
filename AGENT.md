...
- 参考音频：常见音频格式均收（wav/mp3/flac/ogg/opus/m4a/aac/wma/webm/aiff），解码统一走 `common/audio_codec.py`（与主干共用：soundfile 主解码 + ffmpeg 兜底），上传后统一转 PCM_16 WAV 落盘、展示名归一为 .wav；5–10s 清晰人声，**必须填参考文本**（说了什么），否则克隆质量明显下降
- 音色打包：`GET /api/clone/refs/{id}/export` 导出 `.clone`（ZIP：voice.json + ref.wav，含参考文本与预设合成文本，**不含模型权重**）；`POST /api/clone/refs/import` 导入还原
...
| 2026-09-09 | 克隆链路四项优化：① 采集设备下拉默认改为「系统默认」（不再自动选中后端 default_index）② 删除 ClonePanel「保存参考文本」按钮（参考文本上传时填写 + 合成时可临时覆盖，PATCH 端点保留）③ **上传转码统一**：新增 `common/audio_codec.py`（soundfile 主解码 + ffmpeg 兜底 + 编码 WAV，主干与适配层共用同一份实现）；主干 `/api/audio/upload` 从仅 soundfile 扩到全部常见格式（m4a/aac/wma/webm 不再 415），两侧上传后展示名统一归一为 .wav（转换可见）；解码策略改为「直接读失败 → 先 ffmpeg（干净处理 VBR mp3 尾帧/坏头 flac/m4a moov-at-end）→ 再分块截断兜底」，修复管道 FLAC 声明帧数异常导致的静默截断 ④ **音色 .clone 保存/导入**：ZIP 容器（voice.json + ref.wav）含参考音频+参考文本+预设合成文本（取该音色最近一次合成文本，合成时自动记录），不含模型权重；适配层 export/import 端点、主干 TTSProvider 抽象扩 2 方法 + 路由代理、前端导出按钮（blob 下载 + Content-Disposition 文件名）与 .clone 导入入口、选中音色自动预填合成文本；新增测试 10 项（roundtrip/非法包/代理/503/文件名归一），pytest 72 → 82 项全过，ruff 通过，前端 build 通过 | zahiko |
| 2026-09-08 | 参考音频多格式支持：`services/clone` 上传从仅 WAV 放宽为 wav/mp3/flac/ogg/opus/m4a/aac/wma/webm/aiff——soundfile(libsndfile) 主路径解码，解不动的格式走 ffmpeg 兜底（管道直解优先，mp4 系 moov-at-end 不可 seek / 0 帧空输出时自动退临时文件输入重试；ffmpeg 查找顺序 `CLONE_FFMPEG` → 引擎整合包 → 系统 PATH），最终统一转写 PCM_16 WAV 落盘，引擎侧零感知；前端 ClonePanel accept 放行全格式并加格式提示；新增 `tests/test_clone_refs.py` 13 项（含 ffmpeg 真转码的 m4a/aac/mp3/wma/flac/ogg 用例，无 ffmpeg 自动跳过）；pytest 59 → 72 项全过，ruff 通过，前端 build 通过 | zahiko |
...
| 2026-09-09 | 前端布局与 UI 重构：① 拓展功能改为底部卡片入口（默认收起，防页面拥挤），语音克隆卡片带服务状态徽标、点开侧滑抽屉承载 ClonePanel，合成成功自动关闭回流列表；预留质量检测 / 加噪降噪两张「开发中」卡片 ② 频谱与语谱图合并为标签页面板，消除纵向堆叠 ③ 全局设计系统翻新（style.css：配色/按钮/表单/面板标题强调条/滚动条/焦点态）④ 顶栏品牌化（图标 + 状态胶囊）、播放器条与录音按钮（脉冲红点）、音频列表（选中态光条）精修；零逻辑改动、零契约变更，前端 build 通过 | zahiko |
| 2026-09-09 | 前端视觉重设计为「编辑级录音室」浅色主题：暖纸底 + 墨色文字 + 朱红点缀 + 衬线刊头（墨块「声」字标、粗墨分隔线、衬线编号拓展卡片 01/02/03）；三个画布改读 `--scope-*` CSS 变量（换主题不改组件），波形/频谱/语谱图保持深底琥珀「示波器屏幕」与浅色页面形成硬件控制台对比；按钮改细线幽灵风、主按钮改墨块；分析标签改下划线式；零逻辑/契约变更，前端 build 通过 | zahiko |
| 2026-09-09 | 克隆合成参数扩展 + 记忆式默认：引擎可调参数从 3 项扩到 11 项——新增文本切分 `text_split_method`（cut0–cut5）、批大小 `batch_size`、句间停顿 `fragment_interval`、采样温度 `temperature`、`top_k`/`top_p`、重复惩罚 `repetition_penalty`、随机种子 `seed`（默认值与引擎 api_v2 `TTS_Request` 对齐，主干/适配层 schema 镜像透传）；ClonePanel 新增「高级参数」折叠区，每项括号注明效用；参数与选中参考音走 localStorage 记忆（`dzxt.clone-params.v1` / `dzxt.clone-last-ref.v1`），下次打开沿用上一次的值，另有「恢复默认参数」；新增测试 3 项（主干透传/默认兼容/适配层 payload 转发），pytest 82 → 85 项全过，ruff 通过，前端 build 通过 | zahiko |
| 2026-09-09 | 修复引擎警告「Prompt free is not supported batch_infer!」：根因为参考文本为空时引擎 prompt-free 模式不支持批量，`batch_size>1` 被静默降级为逐句推理——适配层在有效参考文本为空时强制归一 `batch_size=1`（消除无效批量与告警噪音），前端批大小标签注明「仅填写参考文本时生效」；另修复离线降级测试的环境依赖（假设 9900 无监听，本机适配层运行时被真服务穿透）——`client` fixture 改为注入指向 `127.0.0.1:1` 的真 Provider，离线路径可复现；pytest 85 → 86 项全过，ruff 通过，前端 build 通过 | zahiko |
